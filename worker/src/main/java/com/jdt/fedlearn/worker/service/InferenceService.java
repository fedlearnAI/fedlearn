...
public class InferenceService {
    ...
    public String push(PutRemote content) {
        ...
        try {
            ...
            String filePath = StringUtil.isBlank(fileSuffix) ? path + newFileName : path.replace(dot + fileSuffix, newFileName + dot + fileSuffix);
            push(picArray, filePath);
            resPath += SUCCESS_SUFFIX;
        } catch (Exception e) {
            resPath += FAIL_SUFFIX;
    ...
    private void push(List<SingleInference> picArray, String filePath) {
    ...
    public Map<String, Object> validationMetric(Map content) {
        ...
        try {
            ...
            String labelName = (String) content.get("labelName");
            ArrayList metricTypes = (ArrayList) content.get("metric");
            String res = getMetric(picArray, labelName, metricTypes);
    ...
    private String getMetric(List arrayList, String labelName, ArrayList metricTypes) {
        //todo master传过来label的特征名
        Map labelMap = TrainDataCache.loadLabelMap(labelName);
        if (null == labelMap) {
            return "no_label";
        }
        ...
        for (int i = 0; i < metricTypes.size(); i++) {
            if (Arrays.asList(arr).contains(metricTypes.get(i))) {
