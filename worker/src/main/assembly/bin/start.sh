...
SERVER_NAME=`grep -v '^\s*#' conf/worker.properties | sed '/app.name/!d;s/.*=//' | tr -d '\r'`
SERVER_PORT=`grep -v '^\s*#' conf/worker.properties | sed '/app.port/!d;s/.*=//' | tr -d '\r'`
APP=`sed '/"APP"/!d;s/.*value="\(.*\)".*/\1/' conf/logback.xml`
eval LOGS_DIR=`sed '/"LOG_HOME"/!d;s/.*value="\(.*\)".*/\1/' conf/logback.xml`


CONFIG=""
DEFAULT_CONFIG="/export/Config/worker.properties"
JAVA_DEBUG_OPTS=""
...
MAIN_CLASS="com.jdd.ml.federated.client.WorkerHttpApp"
