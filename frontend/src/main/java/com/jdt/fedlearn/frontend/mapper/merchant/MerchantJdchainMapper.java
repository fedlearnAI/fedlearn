...
public class MerchantJdchainMapper{
    ...
    */
    public List<Merchant> queryAllMerchant() {
        List<Merchant> reslt = new ArrayList<>();
        ...
        if(typedKVEntries != null){
            reslt = Arrays.stream(typedKVEntries)
                    .map(typedKVEntry -> JsonUtil.json2Object((String) typedKVEntry.getValue(), Merchant.class))
    ...
    * @date: 2021/3/8 10:03 上午
    */
    public Merchant queryMerchantById(String merCode){
        String typedKVEntries = jdChainBaseMapper.queryLatestValueByKey(merchantTableAddress, merCode);
        Merchant merchant = JsonUtil.json2Object(typedKVEntries, Merchant.class);
        return merchant;
    }
}
