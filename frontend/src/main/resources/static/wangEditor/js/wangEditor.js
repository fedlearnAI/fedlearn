(function(factory) {
    ...
    Menu.fn.updateSelected = function() {
      ...
      if (!updateSelectedEvent) {
        ...
        updateSelectedEvent = function() {
          ...
          if (commandValue) {
            if (
              editor.queryCommandValue(commandName).toLowerCase() === commandValue.toLowerCase()
            ) {
              return true;
            }
  ...
  _e(function(E, $) {
    ...
    DropList.fn._position = function() {
      ...
      if (editor._isMenufixed) {
        top =
          top + ($menuContainer.offset().top + $menuContainer.outerHeight() - $list.offset().top);
  ...
  _e(function(E, $) {
    ...
    DropPanel.fn._position = function() {
      ...
      if (editor._isMenufixed) {
        top =
          top + ($menuContainer.offset().top + $menuContainer.outerHeight() - $panel.offset().top);
    ...
    Txt.fn.bindPasteFilter = function() {
          ...
          if (data && data.getData) {
            ...
            if (pasteHtml) {
              ...
              if (pasteHtml) {
                ...
                resultHtml = '<p>' + pasteHtml + '</p>';
                // 查询链接
                resultHtml = resultHtml.replace(/<p>(https?:\/\/.*?)<\/p>/gi, function(
                  match,
                  link
                ) {
      ...
      function handle(elem) {
        ...
        if (legalTagArr.indexOf(nodeName) >= 0) {
          ...
          // 忽略的标签
          if (
            ['meta', 'style', 'script', 'object', 'form', 'iframe', 'hr'].indexOf(nodeName) >= 0
          ) {
            return;
          }
          // 其他标签，移除属性，插入 p 标签
  ...
  _e(function(E, $) {
    ...
    Txt.fn.initHeight = function() {
      ...
      // 限制最小为 50px
      txtHeight = txtHeight < 220 ? 220 : txtHeight;
    ...
    Txt.fn.updateValueEvent = function() {
      ...
      $txt.on(txtChangeEventNames, function(e) {
        ...
        timeoutId = setTimeout(doOnchange, 10);
      });
    };
    ...
    Txt.fn.showHeightOnHover = function() {
      ...
      $txt
        .on('mouseenter', 'ul,ol,blockquote,p,h1,h2,h3,h4,h5,table,pre', function(e) {
  ...
  _e(function(E, $) {
    ...
    // 上传图片的配置
    // server地址
    //E.config.uploadImgUrl = '/api/system/conf/picture/upload';
  ...
  _e(function(E, $) {
    ...
    E.UI.menus = {
      ...
      templateBtn: {
        normal: '<a href="#" tabindex="-1"><i class="wangeditor-menu-img-wrench"></i></a>',
        selected: '.selected',
      },
      templateMRC: {
        normal:
          '<a href="#" tabindex="-1" id="mrcButton"><i class="wangeditor-menu-img-search-plus"></i></a>',
        selected: '.selected',
      },
    };
  });
  ...
  _e(function(E, $) {
    ...
    E.createMenu(function(check) {
      ...
      mapData.loadMapScript = function() {
        ...
        script.src =
          'https://api.map.baidu.com/api?v=2.0&ak=' + ak + '&s=1&callback=baiduMapCallBack'; // baiduMapCallBack是一个本地函数
        try {
          // IE10- 报错
      ...
      var $btnCancel = $('<button class="right gray">' + lang.cancel + '</button>');
      var $checkLabel = $(
        '<label style="display:inline-block;margin-top:10px;color:#666;"></label>'
      );
      var $check = $('<input type="checkbox">');
      $checkLabel
        .append($check)
        .append(
          '<span style="display:inline-block;margin-left:5px;">  ' + lang.dynamicMap + '</span>'
        );
      $btnContainer
        .append($checkLabel)
        .append($btnSubmit)
      ...
      $btnSubmit.click(function(e) {
        ...
        if (isDynamic) {
          ...
          //插入iframe
          iframe =
            '<iframe class="ueditor_baidumap" src="{src}" frameborder="0" width="' +
            sizeWidth +
            '" height="' +
            sizeHeight +
            '"></iframe>';
  ...
  _e(function(E, $) {
    ...
    E.createMenu(function(check) {
      ...
      menu.clickEvent = function(e) {
        ...
        var menuContainer = editor.menuContainer;
        $txt.height(
          E.$window.height() - menuContainer.height() - (txtOuterHeight - txtHeight) // 去掉内边距和外边距
        );
      ...
      fns.onload ||
        (fns.onload = function(resultText, xhr) {
          ...
          try {
            ...
            var img;
            if (resultText.code === 'SUCCESS') {
              E.log('上传成功，即将插入编辑区域，结果为：' + resultText.obj.url);
              // 将结果插入编辑器
              ...
              img.onload = function() {
                var html =
                  ...
                  '" alt="' +
                  originalName +
                  '" class="robot-img focus-img"/>';
                editor.command(null, 'insertHtml', html);
                E.log('已插入图片，地址 ' + resultText.obj.url);
                img = null;
              };
              ...
              img.src = resultText.obj.url;
            } else {
              // 提示错误
              E.warn('上传失败：' + resultText.obj);
              alert(resultText.obj);
            }
          } catch (err) {
            console.warn(err);
            alert('上传失败，请稍后重试');
  ...
  _e(function(E, $) {
    E.plugin(function() {
      ...
      // 自定义UI，并添加到上传dom节点上
      var $uploadIcon = $(
        '<div class="upload-icon-container"><i class="wangeditor-menu-img-upload"></i><p style="font-size: 14px">上传图片小于2MB</p><p style="font-size: 14px">图片宽度小于1200px</p></div>'
      );
      ...
      reader.onload = function(e) {
        ...
        editor.xhrUploadImg({
          ...
          loadfn: function(resultText, xhr) {
            ...
            var editor = this;
            if (width > 1200) {
              alert('上传图片宽' + width + 'px，超过限制宽度1200px，请重新选择图片');
              return;
            }
    ...
    E.plugin(function() {
      ...
      $currentTxt
        ...
        .on('click', 'img', function(e) {
          ...
          // 渲染   点击图片显示给图片设置样式，由于要用到jtalk不能自带样式过去，所以暂时关闭该功能//
          // render();
