...
public class JdChainUtils {
    ...
    public static void initGateway(){
        ...
        try {
            ...
            logger.error("init gateway server error:"+e.getMessage());
        }
        ...
        GatewayServiceFactory serviceFactory = GatewayServiceFactory.connect(gatewayIp,
    ...
    private static void invokeRegister(String ip,String type,String aesKey){
        ...
        try {
            ...
            logger.error("invokeRegister error!",e);
        }
        return;
    }
     ...
     * 随机服务端发起训练
     * */
    public static TransactionResponse invokeRandomtraining(String userName,String taskId,String model,String modelArgs){
        Map<String,Object> metaMap = new HashMap<>();
        metaMap.put("userName",userName);
        metaMap.put("taskId",taskId);
        metaMap.put("model",model);
        metaMap.put("modelArgs",modelArgs);
        String fname = JdChainConstant.INVOKE_RANDOM_TRAINING;
        String putKey = fname + JdChainConstant.SEPARATOR + taskId + JdChainConstant.SEPARATOR + JdChainConstant.CLIENT;
    ...
    public static TransactionResponse invokeStarttraining(String tokenId,String phase,String phaseArgs){
        ...
        String putKey = fname + JdChainConstant.SEPARATOR + JdChainConstant.CLIENT + JdChainConstant.SEPARATOR + tokenId + JdChainConstant.SEPARATOR + phase;
        String eventName = fname;
        try {
            TransactionResponse response = putByChaincode(eventName,fname,putKey,metaMap);
            return response;
    ...
    public static TransactionResponse invokeSummarytraining(String taskId,String phase,String result){
        ...
        String putKey = fname + JdChainConstant.SEPARATOR + taskId + JdChainConstant.SEPARATOR + phase + JdChainConstant.CLIENT;
        String eventName = fname;
     ...
     * */
    public static TransactionResponse putByChaincodeNoEvents(String annotationName,String putKey,ArrayList<String> putVal) throws InvocationTargetException, IllegalAccessException {
        // 新建交易
    ...
    public static TransactionResponse putByChaincode(String eventName,String annotationName,String putKey,Map<String,Object> putVal) throws UnsupportedEncodingException {
        TransactionTemplate txTemp = blockchainService.newTransaction(ledgerHash);
        ContractEventSendOperationBuilder builder = txTemp.contract();
        byte[] putValue = getMapToString(putVal).getBytes(StandardCharsets.UTF_8); //map to string
        builder.send(contractAddress,annotationName,
                new BytesDataList(new TypedValue[]{
                        TypedValue.fromText(dataAccountAddress),
                        TypedValue.fromText(ledgerHash.toBase58()),
    ...
    private static TransactionResponse commit(TransactionTemplate txTpl) {
        ...
        try {
            ...
            logger.error("PreparedTransaction close error:{}",e.getMessage());
        }
        return commit;
    }
     ...
     *  value : 向链上写入的value值
     * */
    private static TransactionResponse saveKV(String dataAccountAddr,String key,String value){
     ...
     *  key : 向链上写入的key值
     * */
    private static String queryLatestValueByKey(String dataAccountAddr,String key){
     ...
     *  key : 向链上写入的key值
     * */
    private static TypedKVEntry[] queryHistoryValueByKey(String dataAccountAddr,String key){
        String[] checkParams = new String[]{dataAccountAddr,key};
        if (!verifyParamsIsNull(checkParams)){
            return null;
        }
