...
/**
 * HdfsReader 从hdfs获取数据
 *
 * @author geyan
 * @version 0.9.2 2021/4/12 10:54 上午
 */
public class HdfsReader implements DataReader {
    ...
    public String[][] loadInference(DataSourceConfig config, String[] uid) {
        HdfsSourceConfig sourceConfig = (HdfsSourceConfig) config;
        String uri = sourceConfig.getUri();
        String user = sourceConfig.getUser();
        ...
        try (
                FileSystem fileSystem = FileSystem.get(URI.create(uri), conf, user);
                InputStream inputStream = fileSystem.open(dst);
                BufferedReader br = new BufferedReader(new InputStreamReader(inputStream, StandardCharsets.UTF_8));
            ){
            //根据uid列表，从文件中加载数据，
    ...
    public String[] loadHeader(DataSourceConfig config) {
        ...
        String[] res;
        Configuration conf = new Configuration();
        Path dst = new Path(sourceConfig.getTrainBase() + sourceConfig.getDataset());
        try (
                FileSystem fileSystem = FileSystem.get(URI.create(uri), conf, user);
                InputStream inputStream = fileSystem.open(dst);
                BufferedReader br = new BufferedReader(new InputStreamReader(inputStream, StandardCharsets.UTF_8));
