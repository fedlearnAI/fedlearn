...
SERVER_PORT=`grep -v '^\s*#' conf/manager.properties | sed '/app.port/!d;s/.*=//' | tr -d '\r'`
APP=`sed '/"APP"/!d;s/.*value="\(.*\)".*/\1/' conf/logback.xml`
eval LOGS_DIR=`sed '/"LOG_HOME"/!d;s/.*value="\(.*\)".*/\1/' conf/logback.xml`


CONFIG=""
DEFAULT_CONFIG="/export/Config/manager.properties"
JAVA_DEBUG_OPTS=""
...
MAIN_CLASS="com.jdd.ml.federated.manager.ManagerHttpApp"
