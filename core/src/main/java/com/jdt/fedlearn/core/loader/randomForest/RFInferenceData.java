...
//                int size = newContent.get(newContent.size()-1).size();
    ...
    public SimpleMatrix selectToSmpMatrix(String[] uids, String labelName) {
        int labelIndex = 0;
        for (int i = 0; i < featureName.length; i++) {
            if (featureName[i].equals(labelName)) {
                labelIndex = i;
                break;
            }
        }
...
//                int size = newContent.get(newContent.size()-1).size();
        ...
        SimpleMatrix mat = new SimpleMatrix(content.size(), content.get(0).size()-1);
        for (int row = 0; row < content.size(); row++) {
            int afterLabel = 0;
            for (int col = 0; col < content.get(0).size(); col++) {
                if (col == labelIndex) {
    ...
    public double[] getLabel(String[] uids, String labelName) {
        int labelIndex = 0;
        for (int i = 0; i < featureName.length; i++) {
            if (featureName[i].equals(labelName)) {
                labelIndex = i;
        ...
        double[] validateLabel = new double[uids.length];
        for (int row = 0; row < content.size(); row++) {
            validateLabel[row] = content.get(row).get(labelIndex);
        }
