 ...
 */
public class DataFrame extends AbstractTrainData implements TrainData {
    private static final Logger logger = Logger.getLogger(DataFrame.class.getName());

    // uid 相关
    private String[] uid;
    private int indexUid; // uid 对应的数据集的 column index
    private String labelUid = "uid"; // uid 对应数据集的 column label
    ...
    public DataFrame(String[][] rawTable) {
    ...
    public DataFrame(String[][] rawTable, List<String> categorical_features) {
        // 构造函数，读取 rawTable 和 categorical features
        headers = new ArrayList<String>();
        content = new ArrayList<ArrayList<Double>>();
    ...
    public DataFrame(String[][] rawTable, String[] idMap, Features features) {
        // 构造函数，只读取 rawTable
