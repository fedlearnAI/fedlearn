...
public class StochasticGD implements Optimizer {

    private final double learningRate;

    public StochasticGD(double lr) {
    ...
    public double[][] getGlobalUpdate(double[][] gredients) {
        for (int i = 0; i < gredients.length; i++) {
            for (int j = 0; j < gredients[i].length; j++) {
                gredients[i][j] = -learningRate * gredients[i][j];
            }
        }
        return gredients;
    }
    ...
    public double[] getGlobalUpdate(double[] gredients) {
        for (int i = 0; i < gredients.length; i++) {
            gredients[i] = -learningRate * gredients[i];
        }
        return gredients;
    }
