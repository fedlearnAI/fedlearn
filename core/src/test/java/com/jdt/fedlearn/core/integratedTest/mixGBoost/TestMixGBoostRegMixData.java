 ...
 * TODO 在测试中增加交叉验证选项，
 */
public class TestMixGBoostRegMixData {
    //服务端维护
    private final String modelToken = "51" + "_" + AlgorithmType.MixGBoost;
    private static final MetricType[] metrics = new MetricType[]{MetricType.MAPE};
    private static MixGBParameter parameter = new MixGBParameter(1.0,1.0, 10, 1, 0, ObjectiveType.regSquare,metrics, 3,20,1,0.3,33,0.6,"", 512);
    ...
    private final String baseDir = "./src/test/resources/regressionD/";
    ...
    public void init() {
        ...
        for (int i = 0; i < partnerSize; i++) {
            ...
            String inferenceFileName = "train" + i + ".csv";
...
//            System.out.println(x.getKey().url() + " =-S1C=-=-: " + boostModel.s1C);
    ...
    public void testInference() throws IOException {
        ...
        String[] predictUid = DataParseUtil.loadInferenceUidList(baseDir + "train2.csv");
//        String[] predictUid = new String[]{"291", "292", "293", "294", "295", "296"};
        System.out.println("predictUid.length " + predictUid.length);
//        double[] label = Arrays.stream(DataLoad.loadInferenceLabelList(baseDir + "train0.csv")).mapToDouble(Double::doubleValue).toArray();
...
//            double value = Metric.calculateMetric(metricType, result, label);
...
//        }
    ...
    public static void main(String[] args) throws IOException {
        TestMixGBoostRegMixData mixGBoostRegMixData = new TestMixGBoostRegMixData();
        mixGBoostRegMixData.init();
        mixGBoostRegMixData.testTrainAndTest();
        mixGBoostRegMixData.testInference();
    }
}
